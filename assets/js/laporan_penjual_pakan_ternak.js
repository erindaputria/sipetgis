...
function printWithCurrentData() {
    ...
    printWindow.document.write('.badge-ya { background-color: #e8f5e9; color: #2e7d32; padding: 2px 8px; border-radius: 12px; display: inline-block; }');
    printWindow.document.write('.badge-tidak { background-color: #ffebee; color: #c62828; padding: 2px 8px; border-radius: 12px; display: inline-block; }');
    ...
    // ========== 2. TABEL DETAIL PENJUAL PAKAN ==========
    printWindow.document.write('<h3 style="margin-top: 20px; color: #832706;">A. DATA PENJUAL PAKAN TERNAK</h3>');
    ...
    if ($('#rekapKecamatanTable').length > 0) {
        ...
        printWindow.document.write('<h3 style="color: #832706;">B. REKAP PENJUAL PAKAN PER KECAMATAN</h3>');
        printWindow.document.write('</div>');
...
function updateRekapTable(data, totals) { 
    if(!data || data.length === 0) {
        $('#rekapTableBody').html('<tr><td colspan="3" class="text-center text-muted">Tidak ada数据<td style="display:none"></td><td style="display:none"></td></tr>');
    ...
    for(var i = 0; i < data.length; i++) {
        var item = data[i];
        var jumlah = parseInt(item.jumlah_usaha) || 0;
        totalUsaha += jumlah;
