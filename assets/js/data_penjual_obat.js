...
function loadDataFromServer() {
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            ...
            $.ajax({
                url: base_url + 'data_penjual_obat/get_all_data',
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    if (response && !response.error) {
...
function renderTable(data) {
    ...
    if (data && data.length > 0) {
        ...
        html = '<tr><td colspan="9" class="text-center py-5"><i class="fas fa-capsules fa-3x text-muted mb-3 d-block"></i>Tidak ada data penjual obat hewan</td><td style="display:none">1</td><td style="display:none">2</td><td style="display:none">3</td><td style="display:none">4</td><td style="display:none">5</td><td style="display:none">6</td><td style="display:none">7</td><td style="display:none">8</td></tr>';
    }
...
function editData(id) {
    $.ajax({
        url: base_url + 'index.php/data_penjual_obat/get_detail/' + id,
        type: 'GET',
        dataType: 'json',
    ...
    if (!map) {
        ...
        setTimeout(function() {
            ...
            currentShopMarker.bindPopup(
                ...
                '<h6 style="margin: 0 0 8px 0; color: #28a745; text-align: center; font-weight: 700;">' + escapeHtml(namaToko) + '</h6>' +
                ...
                '<div><i class="fas fa-capsules fa-fw me-2 text-muted"></i><strong>Kategori:</strong> ' + (kategoriObat || '-') + '</div>' +
        ...
        currentShopMarker.bindPopup(
            ...
            '<div><strong>Kategori Obat:</strong> ' + (kategoriObat || '-') + '</div>' +
            '<div class="text-center mt-2"><small class="text-muted">Klik di luar untuk menutup</small></div>' +
            '</div>'
        ).openPopup();
        mapMarkers.push(currentShopMarker);
