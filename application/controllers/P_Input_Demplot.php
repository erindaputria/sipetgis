...
class P_input_demplot extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('petugas/p_input_demplot', $data);
    }
    ...
    public function save() {
        ...
        $this->form_validation->set_rules('nama_demplot', 'Nama Demplot', 'required|trim');
        $this->form_validation->set_rules('nama_petugas', 'Nama Petugas', 'required|trim');
        $this->form_validation->set_rules('kecamatan', 'Kecamatan', 'required|trim');
        $this->form_validation->set_rules('kelurahan', 'Kelurahan', 'required|trim');
        $this->form_validation->set_rules('alamat', 'Alamat', 'required|trim');
        $this->form_validation->set_rules('luas_m2', 'Luas Lahan', 'required|numeric|greater_than[0]');
        $this->form_validation->set_rules('jenis_hewan', 'Jenis Hewan', 'required|trim');
        $this->form_validation->set_rules('jumlah_hewan', 'Jumlah Hewan', 'required|numeric|greater_than[0]');
        $this->form_validation->set_rules('stok_pakan', 'Stok Pakan', 'required|trim');
        ...
        // Upload foto
        $uploaded_file = null;
        $upload_path = './uploads/demplot/';
        
        if (!is_dir($upload_path)) {
            mkdir($upload_path, 0777, TRUE);
        }

        if (isset($_FILES['foto_demplot']) && $_FILES['foto_demplot']['error'] != 4) {
            ...
            if ($this->upload->do_upload('foto_demplot')) {
        ...
        if ($insert_id) {
            ...
            $response = array(
                'status' => 'success',
                'message' => 'Data demplot berhasil disimpan' . $foto_msg
            );
        } else {
            $response = array(
                'status' => 'error',
                'message' => 'Gagal menyimpan data demplot. Silakan cek kembali data Anda.'
            );
        }
