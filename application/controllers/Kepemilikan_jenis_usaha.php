...
class Kepemilikan_jenis_usaha extends CI_Controller {
    ...
    // Hapus batasan minimal 2 karakter untuk testing
    // if (empty($search) || strlen($search) < 2) {
    //     echo json_encode(['results' => []]);
    //     return;
    // }
    
    // Ambil SEMUA data pelaku usaha tanpa batasan kecamatan
    $this->db->select('nik, nama');
    $this->db->from('pelaku_usaha');
    
    // Jika ada pencarian, filter berdasarkan nama atau nik
    if (!empty($search) && strlen($search) >= 2) {
        $this->db->group_start();
    ...
    $this->db->limit(50); // Tambah limit jadi 50 atau hapus limit sama sekali
    $query = $this->db->get();
    $result = $query->result();
    ...
    public function simpan() { 
        ...
        // Handle nilai kosong untuk gis_lat dan gis_long
        ...
        $data = array(
            ...
            'gis_lat' => (!empty($gis_lat) && $gis_lat !== '') ? $gis_lat : NULL,
            'gis_long' => (!empty($gis_long) && $gis_long !== '') ? $gis_long : NULL
        );
